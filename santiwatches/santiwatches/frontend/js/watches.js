 ...
 * js/watches.js
 *
 * Renderiza la colección de relojes y maneja las acciones del
 * administrador sobre cada card: agregar nuevo reloj y alternar el
 * estado "agotado". El botón de agotado NO borra el producto: solo
 * cambia una bandera (ver watchModel.toggleSoldOut en el backend),
 * que aquí se refleja como una cinta + opacity sobre toda la card.
 */
...
(function () {
  ...
  /**
   * Construye el HTML de una card a partir de los datos de un
   * reloj. Se usa tanto en el render inicial como al agregar uno
   * nuevo, para que ambos caminos generen exactamente la misma
   * estructura (evita duplicar markup con pequeñas diferencias).
   */
   ...
   * evitando que la descripción o el nombre de un reloj puedan
   * inyectar markup arbitrario.
   */
  ...
  /**
   * Delegación de eventos: un único listener en el grid maneja los
   * clics de todos los botones de admin dentro de cualquier card,
   * incluidas las que se agregan dinámicamente después.
   */
  ...
  watchForm.addEventListener('submit', async (event) => {
    ...
    // El precio es opcional: si el campo va vacío, no lo enviamos
    // (el backend lo interpreta como null / "bajo consulta").
  ...
  window.SantiWatches = { loadWatches };
})();
